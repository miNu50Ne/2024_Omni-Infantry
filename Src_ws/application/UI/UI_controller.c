...
static void UI_StaticInit()
{
    ...
    UICharRefresh(&ui_cmd_recv.robot_id_for_ui, Char_State[0]);
    ...
    UICharRefresh(&ui_cmd_recv.robot_id_for_ui, Char_State[1]);
    ...
    UICharRefresh(&ui_cmd_recv.robot_id_for_ui, Char_State[2]);
    ...
    UICharRefresh(&ui_cmd_recv.robot_id_for_ui, Char_State[3]);
    ...
    UICharRefresh(&ui_cmd_recv.robot_id_for_ui, Char_State[4]);
    ...
    UICharRefresh(&ui_cmd_recv.robot_id_for_ui, Char_State[5]);

    UIFloatDraw(&Shoot_Local_Heat, "of1", UI_Graph_ADD, 9, UI_Color_Green, 20, 3, 3, 400, 780, (ui_cmd_recv.Shooter_heat) * 1000);

    // 发送
    UIGraphRefresh(&ui_cmd_recv.robot_id_for_ui, 7, shoot_line[0], shoot_line[1], shoot_line[2], shoot_line[3], shoot_line[4], shoot_line[5], shoot_line[6]);
    UIGraphRefresh(&ui_cmd_recv.robot_id_for_ui, 7, state_circle[0], state_circle[1], state_circle[2], state_circle[3], Cap_voltage, Shoot_Local_Heat);
}
...
void UIDynamicRefresh()
{
    ...
    // 动态UI发送
    UIGraphRefresh(&ui_cmd_recv.robot_id_for_ui, 5, state_circle[0], state_circle[1], state_circle[2], state_circle[3], Cap_voltage);
    UIGraphRefresh(&ui_cmd_recv.robot_id_for_ui, 1, Shoot_Local_Heat);
}
