...
uint8_t UI_Graph_Setting(UI_GRAPH_INSTANCE *instance, RefreshMode_ mode)
{
    ...
    switch (mode) {
        case RefreshMode_ADD:
            Graph_Operate = Graphic_Operate_ADD;
            break;
        case RefreshMode_CHANGE:
            Graph_Operate = Graphic_Operate_CHANGE;
            break;
        case RefreshMode_DELETE:
            Graph_Operate = Graphic_Operate_DEL;
            break;
    }
    switch (instance->type) {
        case GraphType_Rect:
            ...
            UIRectangleDraw(&graph_t, instance->graphic_name, Graph_Operate, instance->layer, instance->color, instance->width, instance->pos_x, instance->pos_y, Rect_end_x, Rect_end_y);
            break;
        case GraphType_Line:
            ...
            UILineDraw(&graph_t, instance->graphic_name, Graph_Operate, instance->layer, instance->color, instance->width, instance->pos_x, instance->pos_y, Line_end_x, Line_end_y);
            break;
        case GraphType_Round:
            UICircleDraw(&graph_t, instance->graphic_name, Graph_Operate, instance->layer, instance->color, instance->width, instance->pos_x, instance->pos_y, instance->param.Round.radius);
            break;
        case GraphType_Elipse:
            UIOvalDraw(&graph_t, instance->graphic_name, Graph_Operate, instance->layer, instance->color, instance->width, instance->pos_x, instance->pos_y, instance->param.Ellipse.radius_x, instance->param.Ellipse.radius_y);
            break;
        case GraphType_Arc:
            ...
            UIArcDraw(&graph_t, instance->graphic_name, Graph_Operate, instance->layer, instance->color, start_angle, end_angle, instance->width, instance->pos_x, instance->pos_y, instance->param.Arc.radius_x, instance->param.Arc.radius_y);
            break;
        case GraphType_Number:
            UIIntDraw(&graph_t, instance->graphic_name, Graph_Operate, instance->layer, instance->color, instance->param.Number.size, instance->width, instance->pos_x, instance->pos_y, instance->param.Number.value);
            break;
        case GraphType_Float:
            ...
            UIFloatDraw(&graph_t, instance->graphic_name, Graph_Operate, instance->layer, instance->color, instance->param.Float.size, instance->param.Float.digit, instance->width, instance->pos_x, instance->pos_y, value);
            break;
        case GraphType_Ray:
            ...
            UILineDraw(&graph_t, instance->graphic_name, Graph_Operate, instance->layer, instance->color, instance->width, Ray_start_x, Ray_start_y, Ray_end_x, Ray_end_y);
            break;
    }
...
void UI_Graph_Refresh(referee_id_t referee_id)
{
        ...
        for (int j = 0; j < graph_idx_priority[i]; j++) {
            ...
            if (graph_instance_group[i][j]->send_flag && --graph_instance_group[i][j]->delay <= 0) {
                ...
                if (UI_Graph_send_buf_cnt == 7) {
                    ...
                    UIGraphRefresh(&referee_id, 7, UI_Graph_send_buf[0], UI_Graph_send_buf[1], UI_Graph_send_buf[2], UI_Graph_send_buf[3], UI_Graph_send_buf[4], UI_Graph_send_buf[5], UI_Graph_send_buf[6]);
                    return;
                }
            }
        }
    ...
    while (UI_Graph_send_buf_cnt < 7) {
        if (graph_instance[idle_refresh_idx]->active_flag == 0) {
            idle_refresh_idx++;
            continue;
        }
...
void UI_String_Refresh(referee_id_t referee_id)
{
    // 按优先级遍历所有UI字符串实例
    for (int i = UI_String_PRIORITY_LEVEL - 1; i >= 0; i--) {
        for (int j = 0; j < string_idx_priority[i]; j++) {
            ...
            if (string_instance_group[i][j]->send_flag && --string_instance_group[i][j]->delay <= 0) {
                ...
                UICharRefresh(&referee_id, string_instance_group[i][j]->string);
                return;
            }
        }
    }
