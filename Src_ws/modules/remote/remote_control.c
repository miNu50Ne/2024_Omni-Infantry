 ...
 * @brief 遥控器数据解析
 *
 * @param dbus_buf 接收buffer
 */
static void dbus_to_rc(const uint8_t *sbus_buf)
{
    // 摇杆,直接解算时减去偏置
...
/**
 * @brief 对dbus_to_rc的简单封装,用于注册到bsp_usart的回调函数中
 *
 */
static void RemoteControlRxCallback()
{
    DaemonReload(rc_daemon_instance);         // 先喂狗
    dbus_to_rc(rc_usart_instance->recv_buff); // 进行协议解析
}
